    ...
    public class ProductRepairAppService : ManagermentAppService, IProductRepairAppService
    {
        ...
        public async Task CreateAsync(CreateUpdateOrderRepairDto input)
        {
            ...
            foreach (var productDto in input.Product_repair)
            {
                ...
                if(!string.IsNullOrEmpty(productDto.PR_code))
                {
                    code = productDto.PR_code;
                }    
                else
                {
                    code = await _customCodeAppService.GenerateCode("MD");
                }    

                var pr_repair = _productRepairManager.CreateAsync(code, productDto.PR_Name, productDto.PR_date_finish, pr_quote
                ...
                await _customCodeAppService.UpdateAsync("MD");
            }    
