...
using Managerment.OrderRepairSales;
...
namespace Managerment.OrderRepairs
{
    ...
    public class OrderRepairAppService : ManagermentAppService, IOrderRepairAppService
    {
                                 ...
                                 DateTime? toDate = null, ECustomerTypeEnum? customerTypeEnum = null, EProcessRepairType? eProcessRepairType = null, Guid? user_id = null)
        {
            ...
            foreach (var item in data_page)
            {
                expected_revenue += item.Fix_price;
            }    
        ...
        public async Task<ReponseDataDto<GroupProcessRepairDto>> GetGroupCustomerProcessRepair(int page, int perPage, string filter = null, DateTime? fromDate = null,
                         DateTime? toDate = null, ECustomerTypeEnum? customerTypeEnum = null, EProcessRepairType? eProcessRepairType = null, Guid? user_id = null)
        {
            var result = await GetListProcessRepair( page,  perPage,  filter ,   fromDate ,  toDate, customerTypeEnum , eProcessRepairType , user_id);

            var group = result.Data.Data_process_repair.ToLookup(x => x.Id_customer).ToDictionary(x => x.Key, x => x.ToList());

            var data = new GroupProcessRepairDto
            {
                Checking_count=result.Data.Checking_count,
                Data_process_repair= group,
                Expected_revenue=result.Data.Expected_revenue,
                Fixed_count=result.Data.Fixed_count,
                Fixing_count=result.Data.Fixing_count,
                Quote_count=result.Data.Quote_count,
                Return_customer=result.Data.Return_customer
            };
            return new ReponseDataDto<GroupProcessRepairDto>
            {
                Page = result.Page,
        ...
        public async Task<ReponseDataDto<GroupProcessRepairDto>> GetGroupUserProcessRepair(int page, int perPage, string filter = null, DateTime? fromDate = null,
                        DateTime? toDate = null, ECustomerTypeEnum? customerTypeEnum = null, EProcessRepairType? eProcessRepairType = null, Guid? user_id = null)
        {
            var result = await GetListProcessRepair(page, perPage, filter, fromDate, toDate, customerTypeEnum, eProcessRepairType, user_id);

            var group = result.Data.Data_process_repair.ToLookup(x => x.Id_user).ToDictionary(x => x.Key, x => x.ToList());
        ...
        public async Task UpdateProcessRepair (Guid process_repair_id , Guid product_repair_id ,Guid order_repair_id,  UpdateProcessProductRepairDto updateProcessProductRepairDto)
        {
            ...
            product_repair.PR_price = updateProcessProductRepairDto.PR_price;
        ...
        //public async Task<List<OrderRepairSaleDto>> GetListRepairSale(string filter , Guid customer_id , Guid store_id, 
